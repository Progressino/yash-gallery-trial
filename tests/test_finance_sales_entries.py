...
def test_create_finance_sales_entries_replaces_existing_rows_for_same_upload(fin_db):
    uid = fdb.create_finance_sales_upload(
        {
            "platform": "Amazon",
            "period": "2026-04",
    ...
    fdb.create_finance_sales_entries(uid, [row])
    # Re-save same upload payload should not append duplicate SUE rows.
    fdb.create_finance_sales_entries(uid, [row])
    day = fdb.get_voucher_summary_by_date("2026-04-01")
