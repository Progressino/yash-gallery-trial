...
if [[ "${DEPLOY_FAST:-}" == "1" ]]; then
  ...
  "$PYTHON_BIN" -m pytest tests/test_daily_sales_auto_refresh.py tests/test_po_sales_lag_and_cache.py tests/test_tier3_prod_window.py tests/test_coverage_light_fast.py tests/test_inventory_clear_and_myntra.py -q
