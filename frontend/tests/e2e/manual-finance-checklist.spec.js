...
test("manual finance checklist across CRONUS menu paths", async ({ page, context, baseURL }) => {
  ...
  const quickOpenExpectations = [
    ...
    ["Inventory", /SKU movement \(Finance\)/],
  ...
  await page.getByRole("button", { name: /^Sales/i }).first().click();
  await page.getByRole("button", { name: "Inventory (shipments / returns)", exact: true }).click();
  await expect(page.getByRole("heading", { name: "SKU movement (Finance)" })).toBeVisible();
